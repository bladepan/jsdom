...
function startTimer(window, startFn, stopFn, callback, ms) {
  // https://github.com/tmpvar/jsdom/issues/917
  var callbackWrap = function () {
    callback.call(window, arguments);
    var timers = window.__timers
    for (var i in timers) {
      if (timers[i][0] === res) {
        timers.splice(i, 1);
        break;
  ...
  var res = startFn(callbackWrap, ms);
...
function Window(document) {
  ...
  this.setTimeout = function (fn, ms) { 
    if (ms == null || ms === 0) {
       //optimize setTimeout(fn, 0)
       return startTimer(window, setImmediate, clearImmediate, fn);
    } 
